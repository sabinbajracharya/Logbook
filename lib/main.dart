...
build(cx)=>((_pCtrl)=>
Scaffold(backgroundColor: Colors.white,body:_l
...
class _VS extends State<VP>{
  var _vc,_vr=false;
  lv(u){_vc=VideoPlayerController.asset(u)..initialize().then((_){_vc..setLooping(true)..play();setState(()=>_vr=true);});}
  @override
  void initState() {
    lv(widget.o['v']);
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      child:Stack(
        ...
        children:[_vr
            ?VideoPlayer(_vc)
            :Image.asset(p['i'],fit: BoxFit.cover,height: 1/0,width: 1/0),
          BackdropFilter(
            child: C(height: 52, color: Colors.white.withOpacity(0.1),
              child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [z(Icons.add_comment),z(Icons.favorite_border)]),
            ),
