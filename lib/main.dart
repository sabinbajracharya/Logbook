...
var cp=1,np=-1,fade=1.0,_l=true,_d,_vr=false,_bc,_sa,_za,_fa,_vc,ICN=Icon(Icons.chevron_right,color:CW);
ss(s)=>setState(s);
lv(u)async{if(_vc!=null)await _vc.dispose();_vc=await vpc(u);ss(()=>_vr=true);}
...
child:InkWell(borderRadius:br(52.0),onTap:(){ss(()=>_vr=false);_vc..pause()..dispose();_bc.forward();},
child:C(width:_za.value==50?_sa.value:_za.value,height:_za.value,
child:Row(mainAxisAlignment:AC,
children:_sa.value<75.0?_za.value<300?[CircularProgressIndicator(strokeWidth:1.0,valueColor:AlwaysStoppedAnimation(CW))]:[]:_sa.value<200.0?[ICN]:[ICN,Text(_d['a'],style:TextStyle(color:CW))])))))
])
:Stack(children:[
Scaffold(
backgroundColor:CW,
